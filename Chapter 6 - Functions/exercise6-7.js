...
Create a countdown loop starting at a dynamic value of 10.
	...
	9. Make sure, when you send the new countdown value as an argument into the
	   function, that there is a way out of the loop by using the return keyword and
	   a condition that continues the loop if met.
*/	   

let start = 10;

function countdown(i) {
	console.log("The countdown value is " + i);
	if (i < 1){
		return;
	} else {
		return countdown(--i);
...
function countdown2(i) {
	console.log("The second countdown value is " + i);
	if (i > 0) {
		i--;
		return countdown2(i);
	}
