...
NOTE: you will need a .html file to implement this program.
...
2. Generate a random number for the solution using Math.random() and 
   ...
   1-[whatever the set max value is]. You can log this value to the console 
   for development to see the value as you create the game, then when the 
   game is complete you can comment out this console output.
...
5. Inside the while loop, check using a condition to see if the prompt value 
   ...
   initiate another prompt until the user guesses correctly. In this way we 
   use the loop to keep asking until the solution is correct, and at that 
   point we can stop the iteration of the block of code.*/  
...
while (!correct) {
   let guessNumber = Number(prompt("Enter a number between 1 and 5: "));
